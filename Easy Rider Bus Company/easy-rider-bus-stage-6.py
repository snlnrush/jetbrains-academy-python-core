...
def check_on_demand(js_obj):
    sf_stops = set()
    o_stops = set()
    for obj in js_obj:
        if obj['stop_type'] in {'S', 'F', ''}:
            sf_stops.add(obj['stop_name'])
        elif obj['stop_type'] == 'O':
            o_stops.add(obj['stop_name'])
    on_demand_stops = sf_stops & o_stops
    if len(on_demand_stops) > 0:
        print('On demand stops test:')
        print(f'Wrong stop type: {sorted(on_demand_stops)}')
    else:
        print('On demand stops test:\nOK')
...
The work seemed tedious and endless, but it finally looks like we're getting there.
There is only one more point left in the specification that we need to check and fix: on-demand stops cannot be initial, final, or transfer stops.
